...
class FormHangman(FormAction):
    ...
    def validate(self, dispatcher, tracker, domain):
        # This function will be called every single time until we allow the
        # "letter" slot to be filled. It will also ignore other policies.
        # ie. we have ourselves a nice loop going.
        hangman = tracker.get_slot("hangman")
        data = {}
        if hangman:
            letter = None
            try:
                letter = tracker.get_slot("letter")[0]
            except Exception:
                return [SlotSet("letter", None)]
            data = json.loads(hangman)
            word = data["word"]
            ...
            # If the game is over unset the "hangman" slot and allow the "letter" slot
            # to be filled. This will cause the "submit" method to be called and exit the
            # loop
            if "_" not in mask:
                dispatcher.utter_template('utter_winner', tracker)
                return [SlotSet("letter", "1"), SlotSet("hangman", None)]
            elif data["incorrect"] > 7:
                dispatcher.utter_template('utter_game_over', tracker)
                return [SlotSet("letter", "1"), SlotSet("hangman", None)]
        else:
            #  Since the hangman slot is not set we can assume this is the first time the form is called
            #  We can now get a random word and initialize the hangman dict
            ...
            data = {"word": word, "guess": [], "incorrect": 0}
            dispatcher.utter_message(" ".join(["_" for l in word]))

        #  Game is still going don't set the "letter" slot
        return [SlotSet("letter", None), SlotSet("hangman", json.dumps(data))]

    def submit(self, dispatcher, tracker, domain):
        # Nothing to do here, the loop ended.
        return []
